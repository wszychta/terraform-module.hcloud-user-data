...
*/
locals {
  netplan_2_network_file = length(var.private_networks_settings) > 0 && var.server_type != "" ? templatefile(
    "${path.module}/config_templates/netplan_2/private_network.tmpl",
    {
  ...
  netplan_2_network_merge_script = length(var.private_networks_settings) > 0 ? templatefile(
  ...
  netplan_2_cloud_config_file = templatefile(
    "${path.module}/config_templates/netplan_2/cloud_init.yaml.tmpl",
    {
      private_network_file_base64          = length(var.private_networks_settings) > 0 ? base64encode(local.netplan_2_network_file) : ""
      private_network_file_path            = local.netplan_2_network_file_path
      network_merge_script_path            = local.netplan_2_network_merge_script_path
      network_merge_script_base64          = length(var.private_networks_settings) > 0 ? base64encode(local.netplan_2_network_merge_script) : ""
