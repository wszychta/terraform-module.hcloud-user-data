  ...
  keyfile_network_config_files_map = length(var.private_networks_settings) > 0 ? [for net_config in var.private_networks_settings :
    {
      ...
      owner       = "root:root"
      path        = "${local.keyfile_network_root_directory}/${local.server_type_letters_only == "cpx" ? "enp${sum([7, index(var.private_networks_settings, net_config)])}s0" : "ens${sum([10, index(var.private_networks_settings, net_config)])}"}.nmconnection"
      permissions = "0600"
    }
  ] : []
